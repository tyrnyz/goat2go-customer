...
- **DB migration:** Added `cancelled_at` (TIMESTAMPTZ) and `cancelled_by` (TEXT, CHECK: 'customer'|'staff'|'system') columns to `orders`. Both nullable, no default. (`cancellation_reason` was also added here but dropped on April 9 — see below.)
